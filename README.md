...
## Responsive — ขนาดหน้าต่างปรับตามจอ (มือถือ / PC)

ทำงานอัตโนมัติ ไม่ต้องตั้งอะไรเพิ่ม หน้าต่างจะ:

1. คำนวณอัตราส่วนที่ทำให้พอดีจอ (หักขอบตาม `FitMargin`)
2. ถ้าจอเล็กกว่า → ย่อทั้ง UI ด้วย `UIScale` (ต่ำสุดไม่เกิน `MinScale`)
3. ถ้าถึง `MinScale` แล้วยังไม่พอ → **ลดขนาดหน้าต่างลงแทน** (เพื่อไม่ให้ตัวอักษรเล็กเกินอ่าน)
4. จัดหน้าต่างกึ่งกลางจอ และ **ทำใหม่ทุกครั้งที่ขนาดจอเปลี่ยน** (หมุนจอ / ย่อขยายหน้าต่าง Roblox)

```lua
local Window = Fluent:CreateWindow({
    Title = "My Hub",
    Size  = UDim2.fromOffset(720, 480),  -- ขนาดที่ "ออกแบบ" ไว้ (ใช้อ้างอิง)
    AutoFit   = true,   -- default true
    MaxScale  = 1,      -- default 1 = ไม่ขยายเกินขนาดที่ออกแบบ (ตั้ง 1.25 ได้ถ้าอยากใหญ่ขึ้นบนจอใหญ่)
    MinScale  = 0.75,   -- default 0.75 = ย่อได้ต่ำสุด 75% หลังจากนั้นจะลดขนาดหน้าต่างแทน
    FitMargin = 32,     -- default 32 = เว้นขอบจอ (px)
    ...
})
```

ผลลัพธ์ที่เทสต์ได้ (รันจริงใน mock):

| จอ | ขนาดหน้าต่าง | สเกล | ที่มองเห็นจริง | คอลัมน์ |
|---|---|---|---|---|
| 1920×1080 (PC) | 720×480 | 100% | 720×480 | 2 |
| 1600×900 | 720×480 | 100% | 720×480 | 2 |
| 820×1180 (แท็บเล็ต) | 720×480 | 100% | 720×480 | 2 |
| 390×844 (มือถือแนวตั้ง) | 477×480 | 75% | **358×360** | **1 (พับ)** |

### คอลัมน์ responsive

เมื่อแท็บแคบกว่า 430px (เช่น มือถือแนวตั้ง) คอลัมน์ซ้าย/ขวาจะ **พับเรียงแนวตั้งเป็นคอลัมน์เดียวอัตโนมัติ**
และจะกลับเป็น 2 คอลัมน์เมื่อกว้างพอ (หมุนจอ / จอใหญ่) — ไม่ต้องเขียนโค้ดเพิ่ม
...
5. **`Components/Window`** — responsive
   - เพิ่ม `UIScale` ให้หน้าต่าง + `Window:Fit()` ที่คำนวณขนาด/สเกล/ตำแหน่งกึ่งกลาง
     และผูกกับ `Camera:GetPropertyChangedSignal("ViewportSize")`

6. **`Components/Tab`** — คอลัมน์ responsive
   - เมื่อแท็บแคบกว่า 430px จะพับคอลัมน์ซ้าย/ขวาเป็นคอลัมน์เดียว (และกลับมา 2 คอลัมน์เมื่อกว้างพอ)

ทดสอบแล้วด้วยการจำลองสภาพแวดล้อม Roblox (ดูโฟลเดอร์ `../testbed`):
คอลัมน์ 16/16, สไลเดอร์ 17/17, รูปทรง 9/9, responsive 20/20 ผ่าน
