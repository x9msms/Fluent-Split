...
| element ทับกัน / ซ่อนอยู่ใต้กล่องอื่น | ไม่มี — Section ขยายความสูงตามเนื้อหาอัตโนมัติ (`AbsoluteContentSize` → `Root.Size`) และคอลัมน์เรียงต่อกันด้วย `UIListLayout` | ถ้าเห็นทับ แสดงว่าใช้โค้ดเรียกผิด (เช่น โยน element ไปคอลัมน์ที่ไม่ได้สร้าง) — ลอง `Window:Fit()` ใหม่ หรือรีโหลด `main.lua` เติม `?t=` |
...
7. **`UICorner`** — ไม่แตะ element corners เดิม whatsoever (โค้งเหมือนเดิมทุกจุด)
...
ที่ความกว้าง 1920 / 1280 / 1024 / 844 / 390 / 640 / 360 px — ผ่านทั้งหมด 58 ช่องตรวจ
