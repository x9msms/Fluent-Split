...
## หน้าต่างทึบ (ไม่ใส) + กรอบหน้าต่างเหลี่ยม
...
### กรอบหน้าต่างเหลี่ยม ที่เหลือโค้งเหมือนเดิม

แค่ **กรอบหน้าต่าง** (window) ที่เหลี่ยม — `Window.Root` กับพื้นหลัง `AcrylicPaint`
ไม่สร้าง `UICorner` ให้เลย จึงเป็นสี่เหลี่ยมออกมาตามธรรมชาติ
（เผชิญหน้า dialog ก็เหลี่ยม เพราะ tint ของมันมีขนาดเท่าหน้าต่างพอดี ถ้าโค้งจะโผล่มุมหน้าต่างออกมา）

ส่วน element ข้างใน **โค้งเหมือนไฟล์เดิมทุกจุด** — ไม่มีการแก้ `CornerRadius` ของ
Toggle / Slider / Dropdown / Colorpicker / Button / Input / Keybind / Paragraph /
Tab / Section / TitleBar / Dialog เลยครับ รวมทั้งวงกลมของ Slider / Toggle / Colorpicker
ที่เดิมใช้ `UDim.new(1, 0)` (กลมสนิท) ก็กลมเหมือนเดิม

ทั้งนี้ยังใช้ `Frame` + `UICorner` ตามเดิม จึงไม่ต้องโหลดรูป asset ใด ๆ
（เดิมโหลดจาก `roblox.com/asset/?id=12266946128` ที่หลายเครื่องโหลดไม่ได้ → รูปแตก）
...
| element ทับกัน / ซ้อนกัน | ไม่มี — Section ขยายความสูงตามเนื้อหา และคอลัมน์เรียงต่อกันด้วย `UIListLayout` | ถ้าเห็นทับ แสดงว่าใช้โค้ดเรียกผิด (เช่น โยน element ไปคอลัมน์ที่ไม่ได้สร้าง) — ลอง `Window:Fit()` ใหม่ หรือรีโหลด `main.lua` เติม `?t=` |
| ปรับหน้าต่างเล็ก แล้ว **มีแผ่นสีเทาบังตัวด้านล่าง** (ของที่เหลือไม่เห็น/กดไม่ได้) | `CanvasSize`/`Size` ของ `ScrollingFrame` กับคอลัมน์เอามาจาก `AbsoluteContentSize` ซึ่งเป็นพิกเซล**จอ** แต่เขียนลงตำแหน่งพิกเซล**หน้าต่าง** — ตอน scale < 1 ค่าที่ได้ เล็กกว่าของจริง `ScrollingFrame` เลยตัดส่วนที่เกินทิ้ง เหลือแต่พื้นหลังหน้าต่าง (สีเทา) โผล่ | แก้แล้ว — ทุกจุดที่อ่าน `AbsoluteContentSize`/`AbsoluteSize` มาเขียนลง `Size`/`CanvasSize` ถูกลบด้วย `Window.Scale` (Section, canvas ของแท็บ, ความสูงสองคอลัมน์, canvas รายการแท็บ) — ตอนนี้ scroll ถึงของทุกตัวที่ทุกขนาด และมีเช็คสำหรับกรณีนี้ใน harness |
...
7. **พิกเซลจอ vs พิกเซลหน้าต่าง** — หลังใส่ `UIScale` ที่ `Root` ค่าที่อ่านจาก `AbsoluteContentSize` / `AbsoluteSize`
   กลายเป็นพิกเซล**จอ** (คูณ scale หมด) แต่ `Size` และ `CanvasSize` เป็นพิกเซล**หน้าต่าง** (ไม่คูณ)
   เดิมโค้ดต้นฉบับเอาตรงๆ ไปเขียนเลย เพราะไม่เคยมี `UIScale` ค่าเดิมโดยดีฟอลต์
   ตอน scale < 1 (หน้าต่างเล็ก) ค่าที่เขียนลงจะ **เล็กกว่าของจริง** → `ScrollingFrame` ตัดส่วนที่เกินทิ้ง
   คอลัมน์ที่ใส่ `ClipsDescendants` ก็ตัด element ล่างสุด → เหลือพื้นหลังสีเทาของหน้าต่างโผล่
   แก้โดยหาร `Window.Scale` ทุกจุดที่สองหน่วยนี้มาบรรจบกัน: Section (`Root.Size`/`Container.Size`),
   canvas ของแท็บ (`ContainerFrame.CanvasSize`), ความสูงสองคอลัมน์ (`CL`/`CR` ใน `APPLY`),
   และ canvas รายการแท็บ (`TabHolder.CanvasSize` ทั้งตัว signal handler และใน `FITCHROME`)
   เหลือตัวที่อยู่นอก `Root` (notification holder กับ dropdown popup — เป็นลูกของ `ScreenGui`)
   ที่พิกเซลจอ = พิกเซลหน้าต่างอยู่แล้ว ไว้อย่างเดิม

8. **`UICorner`** — ไม่แตะ element corners เดิม whatsoever (โค้งเหมือนเดิมทุกจุด)
   เจอการแก้แค่จุดเดียวคือ `UICorner` ของ dialog tint overlay (มันมีขนาดเท่าหน้าต่างพอดี
   ต้องเหลี่ยมตามมิฉะนั้นมุมโค้งของ tint จะโผล่ออกนอกกรอบหน้าต่างที่เหลี่ยม)
...
ครอบคลุม: คอลัมน์ไม่ทับกัน, ไม่มี element ล้นคอลัมน์, ไม่มี element ทับกัน, กรอบหน้าต่างเหลี่ยม + element โค้งเหมือนเดิม,
พื้นหลังทึบ, สไลเดอร์, ลากมุมหน้าต่างจริง (ผ่าน `UserInputService`), และ scale ตรงกับขนาดที่แสดงจริง
ที่ความกว้าง 1920 / 1280 / 1024 / 844 / 390 / 640 / 360 px — ผ่านทั้งหมด 72 ช่องตรวจ
(รวม 14 เช็คใหม่ที่ตรวจว่า `CanvasSize` ของทุกแท็บใหญ่พอให้ scroll ถึงของทุกตัวเมื่อ scale < 1 —
 คือบั๊ก "แผ่นสีเทาบังของด้านล่าง" ที่พบช่วงทดสอบจริง)
